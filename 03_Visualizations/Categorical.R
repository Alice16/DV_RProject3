...
png("./Categoricals.png", width = 25, height = 20, units = "in", res = 72)
grid.newpage()
...
for (i in names(dfR)) {   
  if (i %in% categoricals[[2]]) {
    r <- data.frame(fromJSON(getURL(URLencode('129.152.144.84:5001/rest/native/?query="select \\\""i"\\\" from LEGISLATOR_ROLE where \\\""i"\\\" is not null "'),httpheader=c(DB='jdbc:oracle:thin:@129.152.144.84:1521/PDB1.usuniversi01134.oraclecloud.internal',USER='C##cs329e_yj2946',PASS='orcl_yj2946',MODE='native_mode',MODEL='model',returnDimensions = 'False',returnFor = 'JSON', i=i),verbose = TRUE)))
    p <- myplot1(r,i)
    print(p) 
    l1[[i]] <- p
  }
}
png("./Measures.png", width = 25, height = 10, units = "in", res = 72)
grid.newpage()
